...
**Consulta de cClassTrib por NCM** com arquitetura de dados desacoplada. Este sistema foi meticulosamente recalibrado para a **Reforma Tributária IBS/CBS**, garantindo que empresas e desenvolvedores tenham uma base de referência confiável para a transição tributária brasileira.
...
Diferente das versões anteriores, a V4.0 focou na **higienização dos dados**. Removemos classificações genéricas e corrigimos distorções de alíquotas presentes em versões legadas.
...
| 🛡️ **Sanidade de Dados** | ✅ **OK** | Correção de NCMs de construção e eletrônicos (movidos para Alíquota Integral). |
...
A redução no número de NCMs nesta atualização não é uma perda de conteúdo, mas um **ganho de confiabilidade**. 

1. **Eliminação de Falsos Positivos:** Itens como **Aço (Cap. 72)**, **Cimento (NCM 2523)** e **Eletrônicos (Cap. 85)** estavam incorretamente marcados com 100% de redução. Agora, seguem corretamente a **Regra Geral (CST 000)**.
...
3. **Foco nos Regimes Especiais:** Priorizamos os NCMs que realmente possuem **Redução (60% ou 100%)** ou **Imunidade**, facilitando a consulta de itens que saem da alíquota padrão.
...
* **Anexo VIII:** Sabonetes, cremes dentais, papéis higiênicos e higiene pessoal.
...
* **Design Mobile-First:** Tabelas responsivas com `sticky header` e paginação inteligente.
* **Cores Semânticas:** Diferenciação visual entre itens com Redução (Verde) e Integral (Cinza).
* **LocalStorage Cache:** O sistema armazena o histórico das últimas 10 pesquisas localmente.
* **Tempo de Busca:** ~2ms (O(1) search) com renderização assíncrona.
